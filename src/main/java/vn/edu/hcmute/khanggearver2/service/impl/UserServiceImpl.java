...
public class UserServiceImpl implements UserService {
    ...
    public UserServiceImpl(UserRepository repository, PasswordEncoder passwordEncoder, EmailOtpRepository emailOtps) { this.repository = repository; this.passwordEncoder = passwordEncoder; this.emailOtps = emailOtps; }
    ...
    @Override @Transactional public User update(Long id, User input, Long actorId) { User current = getById(id); normalize(input); rejectDuplicates(input, id); guardSelfUpdate(current, input, actorId); guardLastActiveAdminUpdate(current, input); current.setUsername(input.getUsername()); current.setEmail(input.getEmail()); current.setFullName(input.getFullName()); current.setPhone(input.getPhone()); current.setRole(input.getRole()); current.setActive(input.getActive()); current.setEmailVerified(input.getEmailVerified()); if (input.getPassword() != null && !input.getPassword().isBlank()) current.setPassword(passwordEncoder.encode(input.getPassword())); return repository.save(current); }
    @Override @Transactional public void delete(Long id, Long actorId) { User target = getById(id); guardSelf(target, actorId, "Tu xoa tai khoan"); guardLastActiveAdmin(target, false); if (emailOtps.existsByUserId(id)) throw new BusinessRuleException("Tai khoan da co lich su OTP, hay khoa tai khoan thay vi xoa"); repository.delete(target); }
    ...
    private void guardSelfUpdate(User current, User input, Long actorId) { if (!current.getId().equals(actorId)) return; if (current.getRole() != input.getRole() || !Boolean.TRUE.equals(input.getActive())) throw new BusinessRuleException("Khong duoc tu ha quyen hoac tu khoa tai khoan"); }
    private void guardLastActiveAdminUpdate(User current, User input) { if (current.getRole() == Role.ADMIN && Boolean.TRUE.equals(current.getActive()) && (input.getRole() != Role.ADMIN || !Boolean.TRUE.equals(input.getActive())) && repository.countByRoleAndActiveTrue(Role.ADMIN) <= 1) throw new BusinessRuleException("Khong the lam mat ADMIN active cuoi cung"); }
